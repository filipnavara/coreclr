...
typedef int (*type_coreclr_array_length)(MonoArray* array);
type_coreclr_array_length __coreclr_array_length__;
extern "C" EXPORT_API int coreclr_array_length(MonoArray* array)
{
    log_function_usage("coreclr_array_length");
    if (!__coreclr_array_length__)
        __coreclr_array_length__ = (type_coreclr_array_length)load_mono_function("coreclr_array_length");
    return __coreclr_array_length__(array);
}
...
// Exported Functions for platform PLATFORM_WIN
